...
OG cards are generated by the shared `@alxm/og-image` workspace package (`packages/og-image/`): opentype.js glyph-path text composited with sharp into 1200×630 PNGs. Both sites drive it through a thin `src/_build/og-theme.js` (colours default to the package's navy/cream/pink; dimensions, author footer, portrait, and fonts are site-supplied) plus the package's Eleventy plugin.

- `ogImagePlugin(eleventyConfig, { domain, siteName, generatedDir, defaultImage, theme })` runs on `eleventy.after`: for any page whose `og:image` resolves under `site.og.generatedDir` it renders a per-article card (title recovered from the rendered `og:title`, `titleSeparator`+`siteName` stripped), falling back to the static identity card. Pages opt in by setting `ogImage` in their `.11tydata.js` via `helpers.ogImageUrl(page.url, domain, generatedDir)` — a permalink-mirroring URL, so the file and the meta tag can never disagree. Cards are regenerated every build into `_site` (not committed).
- `scripts/generate-og-image.js` renders the committed static fallback at `src/assets/images/og/default.png` — rerun after theme/portrait/font changes.
- Config: `site.og` (`{ defaultImage, generatedDir, width, height }`); meta tags in `src/_includes/partials/site-opengraph.liquid`.
- alxm.me's footer is name-only; alexmarshalltherapy.com adds a `name · role` line.
