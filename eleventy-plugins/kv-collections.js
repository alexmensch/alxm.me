...
async function fetchKVCollections() {
  ...
  try {
    ...
    const totalItems = Object.values(collections).reduce((sum, collection) => sum + Object.keys(collection).length, 0);
    console.log(`✅ Successfully processed ${totalItems} items across ${Object.keys(collections).length} collection(s)`);
