...
describe("cta shortcode", () => {
  it("renders the body content inside the cta block", () => {
    const html = cta("Book a session with me today.", "/contact", "Contact me");
    assert.ok(html.includes("Book a session with me today."));
    assert.ok(html.includes('class="[ cta ] [ flow flow-space-m ]"'));
  });

  it("renders an anchor with the given href and label", () => {
    const html = cta("Body", "/contact", "Contact me");
    assert.ok(html.includes('<a class="[ button ]" href="/contact">'));
    assert.ok(html.includes("Contact me</a>"));
  });
  ...
  it("renders a plain h2 heading when a title is provided", () => {
    const html = cta("Body", "/contact", "Contact me", "Ready to start?");
    assert.ok(html.includes("<h2>Ready to start?</h2>"));
  });

  it("handles body content containing HTML", () => {
    const html = cta("Text with <em>emphasis</em>", "/contact", "Contact me");
    assert.ok(html.includes("<em>emphasis</em>"));
  });

  it("throws when href is missing", () => {
    assert.throws(() => cta("Body", undefined, "Contact me"), Error);
  });

  it("throws when label is missing", () => {
    assert.throws(() => cta("Body", "/contact", undefined), Error);
  });
});
