...
describe("hasAnyTag", () => {
  it("is true when tags intersect the allow-list", () => {
    assert.equal(
      helpers.hasAnyTag(["psychology", "essay"], ["psychology"]),
      true
    );
  });
  ...
  it("is false when there is no intersection", () => {
    assert.equal(
      helpers.hasAnyTag(["aviation", "travel"], ["psychology"]),
      false
    );
  });

  it("matches against a multi-element allow-list", () => {
    assert.equal(helpers.hasAnyTag(["essay"], ["psychology", "essay"]), true);
  });

  it("is false for empty tags", () => {
    assert.equal(helpers.hasAnyTag([], ["psychology"]), false);
  });
  ...
  it("is false for non-array tags", () => {
    assert.equal(helpers.hasAnyTag(undefined, ["psychology"]), false);
    assert.equal(helpers.hasAnyTag("psychology", ["psychology"]), false);
  });

  it("is false for a non-array allow-list", () => {
    assert.equal(helpers.hasAnyTag(["psychology"], undefined), false);
  });
});
